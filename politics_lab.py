...
def create_voting_dict(strlist):
    ...
    """
    return {_.split()[0]:[int(_.split()[i]) for i in range(3,len(_.split()))] for _ in strlist}
...
def most_similar(sen, voting_dict):
    ...
    dps = {names[i]:policy_compare(sen,names[i],voting_dict) for i in range(len(names)) if names[i]!=sen}
    return sorted(dps, key=dps.get, reverse=True)[0]
...
def least_similar(sen, voting_dict):
    ...
    names = list(voting_dict.keys())
    dps = {names[i]:policy_compare(sen,names[i],voting_dict) for i in range(len(names)) if names[i]!=sen}
    return sorted(dps, key=dps.get)[0]
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    return sum(policy_compare(sen,_,voting_dict) for _ in sen_set)/len(sen_set)
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    return [sum([voting_dict[s][i] for s in sen_set])/len(sen_set) for i in range(len(voting_dict[list(sen_set)[0]]))]
...
def bitter_rivals(voting_dict):
    ...
    for a in voting_dict.keys():
        for b in voting_dict.keys():
            if policy_compare(a, b, voting_dict) < dp:
                dp = policy_compare(a, b, voting_dict)
                br = (a,b)
